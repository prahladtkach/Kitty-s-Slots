// Kitty's Slots Deluxe · versión pública (v2.67) · Tanda 3 de Sugerencias: hilo de debate por sugerencia (subcolección «comments» en suggestions/{id}/comments). Se carga al abrir el hilo; comentar pide el mismo candado que postear/votar; autor o staff pueden borrar. Requiere agregar regla de Firestore para la subcolección. Cache busting → 2.67.
  ...
  // 🦖 Dino (defensivo): ganás menos pero las espinas pegan/aparecen menos. Se desbloquea sobreviviendo una espina enorme.
  const PET_DINO_WIN=0.90;    // ganancias x0.90 (-10%)
  const PET_DINO_DMG=0.65;    // daño de espina x0.65 (-35%)
  const PET_DINO_FREQ=0.70;   // frecuencia de espinas x0.70 (-30%)
  // 😈 Wilson (agresivo, se compra): ganás más pero salen más espinas.
  const PET_WILSON_WIN=1.15;  // ganancias x1.15 (+15%)
  const PET_WILSON_FREQ=1.45; // frecuencia de espinas x1.45 (+45%)
  const PET_WILSON_COST=300;  // cuesta 300 ectofichas
  // 💀 Gaster Blaster (cazador de botes): el jackpot de gatitos paga mas pero las ganancias normales bajan.
  const PET_GASTER_PAY=1.70;  // jackpot 🐱 pagado x1.70 (neto ~+45% tras la contra de -15%)
  const PET_GASTER_WIN=0.85;  // ganancias normales x0.85 (-15%)
  // 🐱 Milson (rachero, se compra): los combos multiplican mas alto pero el jackpot paga menos.
  const PET_MILSON_COMBO=1.5; // techo del multiplicador de combo x1.5 (+50%)
  const PET_MILSON_JP=0.80;   // jackpot 🐱 x0.80 (-20%)
  const PET_MILSON_COST=200;  // cuesta 200 ectofichas
  // 🍓 PIBBLE Frutilla (ahorrador): la caja fuerte rinde mas interes pero ganas menos en la maquina.
  const PET_PIBFRU_INT=1.50;  // tasa de interes del banco x1.50 (+50%)
  const PET_PIBFRU_WIN=0.85;  // ganancias x0.85 (-15%)
  ...
  var PETS={
    dino:{id:'dino',img:'dinosauriogoogle.png',icon:'🦖',name:'Dino',desc:'Sobrevive corriendo sin parar: las espinas pegan menos fuerte y aparecen menos seguido. A cambio, ganás un poco menos.',buff:'🛡️ Espinas -35% daño · -30% frecuencia',contra:'💸 Ganancias -10%',mods:{boneDmg:PET_DINO_DMG,boneFreq:PET_DINO_FREQ,win:PET_DINO_WIN},lines:['RUN','Cactus!','Rawr','No internet :(','Skree','Jump!'],obtain:{via:'logro',logro:'bone_survive'}},
    wilson:{id:'wilson',img:'wilson.png',icon:'😈',name:'Wilson',desc:'El diablito te tienta con plata fácil: ganás más en TODO. A cambio, salen más espinas.',buff:'✨ Ganancias +15%',contra:'🦴 Espinas +45% frecuencia',mods:{win:PET_WILSON_WIN,boneFreq:PET_WILSON_FREQ},lines:['Una mas','Hehehe','Apostá fuerte','Va a salir','Confiá 😈','Mas plata'],obtain:{via:'ecto',cost:PET_WILSON_COST}},
    gaster:{id:'gaster',img:'gasterblaster.png',icon:'💀',name:'Gaster Blaster',desc:'Un láser destructor: el jackpot de gatitos 🐱🐱🐱 paga MUCHO más. A cambio, las ganancias normales bajan.',buff:'🎰 Jackpot 🐱 +45%',contra:'💸 Ganancias normales -15%',mods:{jpPay:PET_GASTER_PAY,win:PET_GASTER_WIN},lines:['...','vrrrr','PEW!','bzzt','grrr','VWOMP'],obtain:{via:'logro',logro:'first_jp'}},
    milson:{id:'milson',img:'milson.png',icon:'🐱',name:'Milson',desc:'Gato con suerte para las rachas: los combos multiplican más alto. A cambio, el jackpot paga menos.',buff:'🔥 Combos multiplican +50% más alto',contra:'🎰 Jackpot 🐱 -20%',mods:{comboCap:PET_MILSON_COMBO,jpPay:PET_MILSON_JP},lines:['¿?','Miau?','Mrrp','¿Eh?','Mishi','Suerte :3'],obtain:{via:'ecto',cost:PET_MILSON_COST}},
