// Kitty's Slots Deluxe · versión pública (v2.35) · Sumados 2 betatesters más (Roli uwu, Dan). Total: 6 betatesters + 2 staff con medalla por UID.
  ...
  const MUSIC_THEMES={ suave:{lead:LEAD_SOFT, wave:'triangle', vol:0.20, base:'soft', nom:'Suave', ver:'v2.13'}, remix:{lead:LEAD_SOFT, wave:'triangle', vol:0.20, base:'dense', nom:'Remix', ver:'v2.27'}, densa:{lead:LEAD_DENSE, wave:'square', vol:0.36, base:'dense', nom:'Densa', ver:'v2.19'} };
  const MUSIC_ORDER=['suave','remix','densa'];
  ...
  function scheduleStep(idx,t){
    ...
    var _TH=MUSIC_THEMES[state.musicTheme]||MUSIC_THEMES.suave; var _ev=_TH.lead[idx];
    if(_TH.base==='soft'){
  ...
  function renderPaytable(){ const el=document.getElementById('ptBody'); const rows=[['🐱🐱🐱','JACKPOT',0],['🐟🐟🐟','x80',0],['💎💎💎','x60',0],['7️⃣7️⃣7️⃣','x40',0],['⭐⭐⭐','x25',0],['🔔🔔🔔','x15',0],['🍒🍒🍒','x10',0],['🍋🍋🍋','x6',0],['🍒🍒·','x4',0],['🍒··','x2',0],['🦴🦴🦴','PIERDES TODO',1],['🦴🦴 sueltas','-doble apuesta+',1],['🦴 suelta','-apuesta',1]]; el.innerHTML=''; rows.forEach(function(r){ const row=document.createElement('div'); row.className='pt-row'; const s=document.createElement('span'); s.style.fontSize='20px'; s.textContent=r[0]; const p=document.createElement('span'); p.className='pay'+(r[2]?' bad':''); p.textContent=r[1]; row.appendChild(s); row.appendChild(p); el.appendChild(row); }); const note=document.createElement('div'); note.style.cssText='margin-top:10px;color:var(--muted);line-height:1.4'; note.innerHTML='COMBO: gana 2+ seguidas y se multiplica (x1.5, x2, x2.5... hasta x'+fmtMult(comboCap())+'). Perder reinicia el combo.<br>🦴 Las espinas pegan más fuerte con cada renacimiento — subí la armadura 🛡️ en el Limbo para compensar.'; el.appendChild(note); }
  ...
  function renderShop(){ checkDiscount(); var el=document.getElementById('upgList'); if(!el) return;
    ...
    var kh='<div class="katto-stall">'+KATTO_STALL_SVG+'<img class="katto-face" id="kattoFace" src="katto-mascot.png" alt="Katto"></div><div class="katto-bubble" id="kattoBubble"></div>';
  ...
  function renderProfile(){ var eq=state.eq; document.getElementById('pfAvatarImg').style.backgroundPosition=pfCatPos(eq.avatar); document.getElementById('pfAvatar').className='pf-avatar '+pfFrameCls(eq.frame); document.getElementById('pfName').textContent=eq.nick||'Jugador'; var tn=pfTitleName(eq.title); document.getElementById('pfTitle').innerHTML = tn? '<span class="title-pill">'+tn+'</span>' : ''; var md=document.getElementById('pfMedals'); if(md){ var meds=userMedals(); if(meds.length){ var mh=''; meds.forEach(function(k){ mh+='<span class="pf-medal" title="'+MEDALS[k].name+' — '+MEDALS[k].desc+'">'+MEDALS[k].svg+'</span>'; }); md.innerHTML=mh; md.style.display='flex'; } else { md.innerHTML=''; md.style.display='none'; } } var L=state.life||{}; document.getElementById('pfEcto').textContent=fmt(L.ecto||0); document.getElementById('pfAsc').textContent=(L.asc||0); document.getElementById('pfRuns').textContent=(L.runs||0); document.getElementById('pfWorth').textContent='$'+fmt(L.bestWorth||0); document.getElementById('pfJP').textContent='$'+fmt(L.bestJP||0); var _pc=document.getElementById('pfCode'); if(_pc) _pc.style.display = (Object.keys(MEDAL_CODES).length>0)?'block':'none'; pfUpdateIcon(); }
  ...
  function finish(t, isRespin, keepMask){
    ...
    if(ch>=1 && chFresh>=1){ combo++; if(combo>(state.life.bestCombo||0))state.life.bestCombo=combo; _tripleStreak=0; const base=(ch===2?4:2)*state.bet; const mult=comboMult(); const w=Math.round(base*mult*winMult()); const net=applyWin(w); updateUI(); setCombo(); setMsg((ch===2?'2 CEREZAS':'1 CEREZA')+(mult>1?' x'+fmtMult(mult):'')+' +'+fmt(w), mult>=2.5?'big':''); celebrate(mult>=2.5); offerGamble(net); saveState(); return; }
  ...
  function closeCat(){ var m=document.getElementById('catModal'); if(m){ m.classList.remove('open'); m.classList.remove('katto'); } _catWho='kitty'; catMode=false; setCatLogin(false); if(_afterCat){ var f=_afterCat; _afterCat=null; setTimeout(f,120); } }
  function showCat(text, who){ var m=document.getElementById('catModal'); if(!m) return; _catWho=who||'kitty'; _introStep=-1; _catWaiting=false; setCatLogin(false); var cont=document.getElementById('catOk'); if(cont) cont.textContent='▶ TOCÁ PARA SEGUIR'; var ci=m.querySelector('.cat-img'); if(ci){ ci.onerror=function(){ this.onerror=null; this.src='cat-mascot.png'; }; ci.src=(_catWho==='katto')?'katto-mascot.png':'cat-mascot.png'; } m.classList.toggle('katto', _catWho==='katto'); m.classList.add('open'); if(state.musicOn){ if(_catWho==='katto'){ shopMode=true; catMode=false; } else { catMode=true; } if(!musicPlaying) startMusic(); } typeCat(text); }
