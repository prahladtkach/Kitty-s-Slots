// Kitty's Slots Deluxe · versión pública (v2.67) · Tanda 3 de Sugerencias: hilo de debate por sugerencia (subcolección «comments» en suggestions/{id}/comments). Se carga al abrir el hilo; comentar pide el mismo candado que postear/votar; autor o staff pueden borrar. Requiere agregar regla de Firestore para la subcolección. Cache busting → 2.67.
  ...
  // Símbolos temáticos por paleta (solo VISUAL — la lógica sigue usando los de siempre). Cambiá los emojis acá; dibujá el sheet para que matcheen.
  const THEME_SYMS={argentina:{'🍋':'🧉','🍒':'🥐','🔔':'🫖','⭐':'💸','7️⃣':'⚽','💎':'💵','🐟':'🇦🇷','🐱':'🏆','🦴':'🟥'}};
  function symDisp(s){ var m=THEME_SYMS[state.palette]; return (m&&m[s])||s; }
  ...
  const PET_DINO_WIN=0.90;    // ganancias x0.90 (-10%)
  const PET_DINO_DMG=0.90;    // daño de espina x0.90 (-10%)
  const PET_DINO_FREQ=0.82;   // frecuencia de espinas x0.82 (-18%)
  const PET_DINO_JP=0.65;     // jackpot 🐱 x0.65 (-35%): le baja el techo
  const PET_DINO_COMBO=0.78;  // techo de combo x0.78 (-22%): le baja el techo
  // 😈 Wilson (explosiva, se compra): ganás mucho más pero salen más espinas (swing alto).
  const PET_WILSON_WIN=1.35;  // ganancias x1.35 (+35%)
  const PET_WILSON_FREQ=1.20; // frecuencia de espinas x1.20 (+20%)
  const PET_WILSON_COST=300;  // cuesta 300 ectofichas
  // 💀 Gaster Blaster (cazador de botes): el jackpot de gatitos paga MUCHO más; ganancias casi sin tocar.
  const PET_GASTER_PAY=2.2;   // jackpot 🐱 pagado x2.2 (+120%)
  const PET_GASTER_WIN=0.97;  // ganancias normales x0.97 (-3%)
  // 🐱 Milson (rachera, se compra): los combos multiplican mas alto pero el jackpot paga un poco menos.
  const PET_MILSON_COMBO=1.5; // techo del multiplicador de combo x1.5 (+50%)
  const PET_MILSON_JP=0.85;   // jackpot 🐱 x0.85 (-15%)
  const PET_MILSON_COST=200;  // cuesta 200 ectofichas
  // 🍓 PIBBLE Frutilla (banquera): la caja fuerte rinde mas interes; ganás un poco menos en la maquina.
  const PET_PIBFRU_INT=1.50;  // tasa de interes del banco x1.50 (+50%)
  const PET_PIBFRU_WIN=0.93;  // ganancias x0.93 (-7%)
  ...
  const PALETTES={
    ...
    argentina:{bg:'#0e2a47',bg2:'#08192e',panel:'#14406a',panel2:'#0f3254',violet:'#74acdf',magenta:'#a9d3f0',cyan:'#5cc4e8',gold:'#f7c52d',green:'#5bd75b',red:'#ff5b79',text:'#f2f8ff',muted:'#9dc0dc',cream:'#eaf4ff',dark:'#05101f'}
  };
  const PALNAMES=['synthwave','gameboy','nes','vapor','lava','mono','galaxy','casino','argentina'];
  ...
  function comboMult(){ if(combo<2) return 1; return Math.min(1+0.5*(combo-1), (3+0.5*state.upg.combo)*petMod('comboCap')); }
  function comboCap(){ return (3 + 0.5*state.upg.combo)*petMod('comboCap'); }
  function fmtMult(m){ return Number.isInteger(m)? (''+m) : m.toFixed(1); }
  var PETS={
    dino:{id:'dino',img:'dinosauriogoogle.png',icon:'🦖',name:'Dino',desc:'El tanque: las espinas pegan menos y salen menos seguido, así te fundís mucho menos. A cambio, tu techo de premios baja (el jackpot y los combos rinden menos).',buff:'🛡️ Espinas -10% daño · -18% frecuencia',contra:'💸 Jackpot -35% · combos -22% · ganancias -10%',mods:{boneDmg:PET_DINO_DMG,boneFreq:PET_DINO_FREQ,win:PET_DINO_WIN,jpPay:PET_DINO_JP,comboCap:PET_DINO_COMBO},lines:['RUN','Cactus!','Rawr','No internet :(','Skree','Jump!'],obtain:{via:'logro',logro:'bone_survive'}},
    wilson:{id:'wilson',img:'wilson.png',icon:'😈',name:'Wilson',desc:'El diablito te tienta con plata fácil: ganás más en TODO. A cambio, salen más espinas.',buff:'✨ Ganancias +35%',contra:'🦴 Espinas +20% frecuencia',mods:{win:PET_WILSON_WIN,boneFreq:PET_WILSON_FREQ},lines:['Una mas','Hehehe','Apostá fuerte','Va a salir','Confiá 😈','Mas plata'],obtain:{via:'ecto',cost:PET_WILSON_COST}},
    gaster:{id:'gaster',img:'gasterblaster.png',icon:'💀',name:'Gaster Blaster',desc:'Un láser destructor: el jackpot de gatitos 🐱🐱🐱 paga MUCHO más. A cambio, las ganancias normales bajan.',buff:'🎰 Jackpot 🐱 +120%',contra:'💸 Ganancias normales -3%',mods:{jpPay:PET_GASTER_PAY,win:PET_GASTER_WIN},lines:['...','vrrrr','PEW!','bzzt','grrr','VWOMP'],obtain:{via:'logro',logro:'first_jp'}},
    milson:{id:'milson',img:'milson.png',icon:'🐱',name:'Milson',desc:'Gato con suerte para las rachas: los combos multiplican más alto. A cambio, el jackpot paga menos.',buff:'🔥 Combos multiplican +50% más alto',contra:'🎰 Jackpot 🐱 -15%',mods:{comboCap:PET_MILSON_COMBO,jpPay:PET_MILSON_JP},lines:['¿?','Miau?','Mrrp','¿Eh?','Mishi','Suerte :3'],obtain:{via:'ecto',cost:PET_MILSON_COST}},
    pibble:{id:'pibble',img:'assets/pibble-mascot.png',themeImg:{argentina:'assets/yosoypibbleche.png'},icon:'🐽',name:'Mr.PIBBLE',desc:'Tu compañero fiel de toda la vida 🤍. No da ningún efecto en el juego — está solo para acompañarte. Tocalo para acariciarlo.',cosmetic:true,mods:{},lines:['Zzz','Im pibble','Wash my belly','Clean my belly','Belly belly'],obtain:{via:'code'}},
  ...
  const LOGROS=[
    ...
    {id:'big_win',cat:'Giros y suerte',icon:'💎',name:'¿Eso estaba balanceado?',desc:'Conseguí un premio de más de $1.000.000 en un giro.',tier:'dificil',secret:false,chk:function(L){return (L.bestJP||0)>=1000000;},prog:function(L){return [L.bestJP||0,1000000];}},
    {id:'good_win',cat:'Giros y suerte',icon:'🍀',name:'Hoy es mi día',desc:'Conseguí un gran premio.',tier:'medio',secret:false,chk:function(L){return (L.bestJP||0)>=50000;},prog:function(L){return [L.bestJP||0,50000];}},
    ...
    {id:'combo10',cat:'Giros y suerte',icon:'😻',name:'Favorito de Kitty',desc:'Ganá 10 veces seguidas.',tier:'medio',secret:false,chk:function(L){return (L.bestCombo||0)>=10;},prog:function(L){return [L.bestCombo||0,10];}},
    {id:'combo25',cat:'Giros y suerte',icon:'🤨',name:'Sospechoso',desc:'Ganá 25 veces seguidas.',tier:'dificil',secret:false,chk:function(L){return (L.bestCombo||0)>=25;},prog:function(L){return [L.bestCombo||0,25];}},
    {id:'combo30',cat:'Giros y suerte',icon:'🎰',name:'Revisen sus dados',desc:'Ganá 30 veces seguidas.',tier:'dificil',secret:false,chk:function(L){return (L.bestCombo||0)>=30;},prog:function(L){return [L.bestCombo||0,30];}},
    {id:'spin1k',cat:'Constancia',icon:'🚪',name:'Una más y me voy',desc:'Realiza 1.000 giros.',tier:'facil',secret:false,count:true,chk:function(L){return (L.spins||0)>=1000;},prog:function(L){return [L.spins||0,1000];}},
    {id:'spin10k',cat:'Constancia',icon:'🤞',name:'Ahora sí, la última',desc:'Realiza 10.000 giros.',tier:'medio',secret:false,count:true,chk:function(L){return (L.spins||0)>=10000;},prog:function(L){return [L.spins||0,10000];}},
    {id:'spin50k',cat:'Constancia',icon:'🤥',name:'Mentiroso',desc:'Realiza 50.000 giros.',tier:'medio',secret:false,count:true,chk:function(L){return (L.spins||0)>=50000;},prog:function(L){return [L.spins||0,50000];}},
    {id:'spin100k',cat:'Constancia',icon:'🔁',name:'Esta vez es diferente',desc:'Realiza 100.000 giros.',tier:'dificil',secret:false,count:true,chk:function(L){return (L.spins||0)>=100000;},prog:function(L){return [L.spins||0,100000];}},
    {id:'spin250k',cat:'Constancia',icon:'😎',name:'Lo tengo controlado',desc:'Realiza 250.000 giros.',tier:'dificil',secret:false,count:true,chk:function(L){return (L.spins||0)>=250000;},prog:function(L){return [L.spins||0,250000];}},
    {id:'spin500k',cat:'Constancia',icon:'🚬',name:'Puedo dejarlo cuando quiera',desc:'Realiza 500.000 giros.',tier:'dificil',secret:false,count:true,chk:function(L){return (L.spins||0)>=500000;},prog:function(L){return [L.spins||0,500000];}},
    {id:'spin1m',cat:'Constancia',icon:'💀',name:'¿Todavía seguís acá?',desc:'Realiza 1.000.000 de giros.',tier:'dificil',secret:false,count:true,chk:function(L){return (L.spins||0)>=1000000;},prog:function(L){return [L.spins||0,1000000];}},
    // Espinas
    ...
    {id:'bone_big',cat:'Espinas',icon:'🐟',name:'Pescado traicionero',desc:'Perdé más de $100.000 por una espina.',tier:'medio',secret:false,chk:function(L){return (L.maxBoneLoss||0)>100000;},prog:function(L){return [L.maxBoneLoss||0,100000];}},
    {id:'bone100',cat:'Espinas',icon:'🙄',name:'¿En serio otra vez?',desc:'Recibí daño de espinas 100 veces.',tier:'medio',secret:false,chk:function(L){return (L.boneHits||0)>=100;},prog:function(L){return [L.boneHits||0,100];}},
    {id:'bone500',cat:'Espinas',icon:'🦔',name:'Erizo honorario',desc:'Recibí daño de espinas 500 veces.',tier:'dificil',secret:false,chk:function(L){return (L.boneHits||0)>=500;},prog:function(L){return [L.boneHits||0,500];}},
    ...
    {id:'asc10',cat:'Limbo',icon:'🗺️',name:'Ya conozco el camino',desc:'Ascendé 10 veces.',tier:'medio',secret:false,chk:function(L){return (L.asc||0)>=10;},prog:function(L){return [L.asc||0,10];}},
    {id:'asc50',cat:'Limbo',icon:'🏠',name:'Dirección permanente',desc:'Ascendé 50 veces.',tier:'dificil',secret:false,chk:function(L){return (L.asc||0)>=50;},prog:function(L){return [L.asc||0,50];}},
    {id:'asc100',cat:'Limbo',icon:'🏚️',name:'Propiedad del Limbo',desc:'Ascendé 100 veces.',tier:'dificil',secret:false,chk:function(L){return (L.asc||0)>=100;},prog:function(L){return [L.asc||0,100];}},
    // Banco
    ...
    {id:'bank1m',cat:'Banco',icon:'💼',name:'Ahorrador profesional',desc:'Guardá $1.000.000.',tier:'medio',secret:false,chk:function(L){return (L.maxBank||0)>=1000000;},prog:function(L){return [L.maxBank||0,1000000];}},
    {id:'bank10m',cat:'Banco',icon:'🏦',name:'Ya casi soy un banco',desc:'Guardá $10.000.000.',tier:'dificil',secret:false,chk:function(L){return (L.maxBank||0)>=10000000;},prog:function(L){return [L.maxBank||0,10000000];}},
    ...
    // Dinero
    {id:'money10k',cat:'Dinero',icon:'🍭',name:'Ya alcanza para un chupetín',desc:'Conseguí $10.000.',tier:'facil',secret:false,chk:function(L){return (L.bestWorth||0)>=10000;},prog:function(L){return [L.bestWorth||0,10000];}},
    {id:'money100k',cat:'Dinero',icon:'👀',name:'Ahí va...',desc:'Conseguí $100.000.',tier:'facil',secret:false,chk:function(L){return (L.bestWorth||0)>=100000;},prog:function(L){return [L.bestWorth||0,100000];}},
  ...
  function checkLogros(silent){
    ...
    for(var i=0;i<LOGROS.length;i++){ var lo=LOGROS[i];
      ...
      if(ok){ L.logros[lo.id]=Date.now(); changed=true; newIds.push(lo.id); if(!silent) queueLogroToast(lo); }
    }
    if(changed) saveState();
    if(newIds.length) bumpLogroStats(newIds);
    syncPetUnlocks(silent);
  }
  function bumpLogroStats(ids){
    if(!(LOGROS_ON && fbReady && fbUser && _fbFsMod && _fbFsMod.increment)) return;
    try{ var upd={}; for(var i=0;i<ids.length;i++){ upd[ids[i]]=_fbFsMod.increment(1); } _fbFsMod.setDoc(_fbFsMod.doc(fbDB,'counters','logroStats'), upd, {merge:true}).catch(function(){}); }catch(_e){}
  }
  ...
  function showLogroToast(lo){
    ...
    el.innerHTML='<div class="lt-ico">'+logroIcoHTML(lo,true,false)+'</div><div class="lt-txt"><div class="lt-top">🏆 ¡LOGRO DESBLOQUEADO!</div><div class="lt-name">'+_rkEsc(lo.name)+'</div></div>';
    wrap.appendChild(el);
  ...
  function fmtLogroNum(cur, goal, isCount){ var c=Math.min(cur||0, goal); if(!isCount && goal>=10000){ return '$'+fmt(c)+' / $'+fmt(goal); } return fmt(c)+' / '+fmt(goal); }
  function logroIcoHTML(g, on, hidden){ if(!LOGRO_IMG_ON) return hidden?'❓':g.icon; var em=hidden?'❓':g.icon; if(!on){ return '<img class="lg-ico-img lg-ico-lock" src="assets/logros/locked.png" alt="" onerror="this.style.display=\'none\';this.nextElementSibling.style.display=\'\'"><span class="lg-ico-em" style="display:none">'+em+'</span>'; } return '<img class="lg-ico-img" src="assets/logros/'+g.id+'.png" alt="" onerror="this.style.display=\'none\';this.nextElementSibling.style.display=\'\'"><span class="lg-ico-em" style="display:none">'+g.icon+'</span>'; }
  var _logTab='mine';
  var _isLocalhost=(function(){ try{ var h=location.hostname; return h==='localhost'||h==='127.0.0.1'||h==='0.0.0.0'||h===''; }catch(e){ return false; } })();
  var _logroPreview=_isLocalhost && /[?&#]preview/i.test((location.search||'')+(location.hash||''));
  if(_isLocalhost){ try{ window.logrosPreview=function(v){ _logroPreview=(v===undefined)?!_logroPreview:!!v; var m=document.getElementById('logrosModal'); if(m&&m.classList.contains('open')) renderLogrosModal(); console.log('[logros] preview =', _logroPreview); return _logroPreview; }; }catch(e){} }
  function openLogros(){ if(spinning) return; var m=document.getElementById('logrosModal'); if(!m) return; _logTab='mine'; m.classList.add('open'); renderLogrosModal(); }
  function renderLogrosModal(){
    var tabs=document.getElementById('logTabs');
    if(tabs){ tabs.innerHTML='<button class="feed-tab'+(_logTab==='mine'?' on':'')+'" data-lt="mine">🏆 Mis logros</button><button class="feed-tab'+(_logTab==='global'?' on':'')+'" data-lt="global">🌎 Globales</button>'; var tb=tabs.querySelectorAll('.feed-tab'); for(var i=0;i<tb.length;i++){ tb[i].addEventListener('click', function(){ _logTab=this.getAttribute('data-lt'); renderLogrosModal(); }); } }
    if(_logTab==='global') renderLogrosGlobal(); else renderLogros();
  }
  function closeLogros(){ var m=document.getElementById('logrosModal'); if(m) m.classList.remove('open'); }
  function renderLogros(){
    var body=document.getElementById('logrosBody'); if(!body) return;
    var L=state.life||{}, unlocked=(L.logros&&typeof L.logros==='object')?L.logros:{};
    var total=LOGROS.length, got=0, k; for(k in unlocked){ if(unlocked.hasOwnProperty(k)) got++; } if(_logroPreview) got=total;
    var pct=total?Math.round(got/total*100):0;
    var h='<div class="lg-summary"><div class="lg-sum-top"><span class="lg-sum-n">'+got+' / '+total+'</span><span class="lg-sum-lbl">desbloqueados</span></div><div class="lg-bar"><div class="lg-bar-fill" style="width:'+pct+'%"></div></div></div>';
    var cats=[], byCat={}, i;
    for(i=0;i<LOGROS.length;i++){ var lo=LOGROS[i]; if(!byCat[lo.cat]){ byCat[lo.cat]=[]; cats.push(lo.cat); } byCat[lo.cat].push(lo); }
    var CATICON={ 'Giros y suerte':'🎰','Constancia':'🔄','Espinas':'🦴','Limbo':'👻','Banco':'🏦','Dinero':'💰','Secretos':'🤫' };
    for(var c=0;c<cats.length;c++){ var cat=cats[c], arr=byCat[cat], cgot=0, a;
      for(a=0;a<arr.length;a++){ if(unlocked[arr[a].id]||_logroPreview) cgot++; }
      h+='<div class="lg-cat"><span>'+(CATICON[cat]||'🏆')+' '+cat+'</span><span class="lg-cat-n">'+cgot+'/'+arr.length+'</span></div><div class="lg-grid">';
      for(var j=0;j<arr.length;j++){ var g=arr[j], on=(!!unlocked[g.id])||_logroPreview, hide=(g.secret&&!on);
        h+='<div class="lg-card'+(on?' on':'')+(hide?' secret':'')+'">';
        h+='<div class="lg-ico">'+logroIcoHTML(g,on,hide)+'</div>';
        h+='<div class="lg-info"><div class="lg-name">'+(hide?'???':_rkEsc(g.name))+'</div><div class="lg-desc">'+(hide?'Logro secreto':_rkEsc(g.desc))+'</div>';
        if(!on && !g.secret && typeof g.prog==='function'){ var pv=g.prog(L,state)||[0,1], cur=pv[0]||0, goal=pv[1]||1, p2=Math.max(0,Math.min(100,Math.round(cur/goal*100))); h+='<div class="lg-pbar"><div class="lg-pbar-fill" style="width:'+p2+'%"></div></div><div class="lg-pnum">'+fmtLogroNum(cur,goal,g.count)+'</div>'; }
        h+='</div>';
        if(on) h+='<div class="lg-check">✓</div>';
        h+='</div>';
      }
      h+='</div>';
    }
  ...
  function renderLogrosGlobal(){
    var body=document.getElementById('logrosBody'); if(!body) return;
    if(!(fbReady&&_fbFsMod&&fbDB)){ body.innerHTML='<div class="pf-rk-msg">🔒 Iniciá sesión para ver las estadísticas globales.</div>'; return; }
    body.innerHTML='<div class="pf-rk-msg">Cargando estadísticas…</div>';
    var pStats=_fbFsMod.getDoc(_fbFsMod.doc(fbDB,'counters','logroStats')).then(function(s){ return (s&&s.exists())?(s.data()||{}):{}; }).catch(function(){ return {}; });
    var pTotal=(_fbFsMod.getCountFromServer&&_fbFsMod.query)? _fbFsMod.getCountFromServer(_fbFsMod.query(_fbFsMod.collection(fbDB,'leaderboard'))).then(function(s){ return (s.data().count||0); }).catch(function(){ return 0; }) : Promise.resolve(0);
    Promise.all([pStats,pTotal]).then(function(res){
      var stats=res[0]||{}, totalP=res[1]||0, L=state.life||{}, unlocked=(L.logros&&typeof L.logros==='object')?L.logros:{};
      var rows=[], i; for(i=0;i<LOGROS.length;i++){ var g=LOGROS[i], cnt=(typeof stats[g.id]==='number')?stats[g.id]:0, pc=(totalP>0)?Math.max(0,Math.min(100,Math.round(cnt/totalP*100))):0; rows.push({g:g,cnt:cnt,pc:pc}); }
      rows.sort(function(a,b){ return b.pc-a.pc || b.cnt-a.cnt; });
      var h='<div class="lg-gl-head">🌎 Cuántos jugadores tienen cada logro<br><span>sobre '+fmt(totalP)+' jugador'+(totalP===1?'':'es')+'</span></div>';
      for(i=0;i<rows.length;i++){ var R=rows[i], gg=R.g, on=(!!unlocked[gg.id])||_logroPreview, hide=(gg.secret&&!on), rare=(R.pc>0&&R.pc<=10);
        h+='<div class="lg-gl-row'+(on?' on':'')+(rare?' rare':'')+'"><div class="lg-ico">'+logroIcoHTML(gg,on,hide)+'</div><div class="lg-gl-info"><div class="lg-name">'+(hide?'???':_rkEsc(gg.name))+'</div><div class="lg-gl-bar"><div class="lg-gl-fill" style="width:'+R.pc+'%"></div></div></div><div class="lg-gl-pct">'+R.pc+'%</div></div>';
      }
      if(!totalP){ h+='<div class="bank-note" style="margin-top:12px">Todavía no hay datos. Las estadísticas se llenan a medida que los jugadores desbloquean logros. 🐱</div>'; }
      body.innerHTML=h;
    });
  }
  ...
  function petImgBase(p){ var f=p.img||(p.id+'.png'); return (f.indexOf('/')>=0)?f:('assets/pets/'+f); }
  function petImgSrc(p){ var f=(p.themeImg&&p.themeImg[state.palette]); if(f) return (f.indexOf('/')>=0)?f:('assets/pets/'+f); return petImgBase(p); }
  function petIcoHTML(p){ return '<img class="pet-ico-img" src="'+petImgSrc(p)+'" alt="" onerror="this.style.display=\'none\';this.nextElementSibling.style.display=\'\'"><span class="pet-ico-em" style="display:none">'+p.icon+'</span>'; }
  function showPetToast(p){ var wrap=document.getElementById('logroToast'); if(!wrap){ wrap=document.createElement('div'); wrap.id='logroToast'; document.body.appendChild(wrap); } var el=document.createElement('div'); el.className='logro-toast'; el.innerHTML='<div class="lt-ico">'+petIcoHTML(p)+'</div><div class="lt-txt"><div class="lt-top">🐾 ¡NUEVA MASCOTA!</div><div class="lt-name">'+_rkEsc(p.name)+'</div></div>'; wrap.appendChild(el); requestAnimationFrame(function(){ el.classList.add('show'); }); setTimeout(function(){ el.classList.remove('show'); el.classList.add('hide'); setTimeout(function(){ if(el.parentNode) el.parentNode.removeChild(el); }, 480); }, 3000); }
  ...
  function refreshAfterLoad(){
    ...
    checkLogros(true); _logrosReady=true; updateDailyIcon(); updatePetsIcon();
  }
  ...
  async function initFirebase(){
    ...
    try{
      ...
      fbReady=true;
      _fbAuthMod.onAuthStateChanged(fbAuth, function(u){ fbUser=u||null; renderAccount(); updatePetsIcon(); if(fbUser){ cloudLoad(); advancePastLogin(); hideLoginToast(); } });
    }catch(e){
      fbReady=false;
  ...
  const sheetAr=new Image(); let sheetArReady=false; sheetAr.onload=function(){ sheetArReady=true; if(state.palette==='argentina' && typeof CV!=='undefined'){ CV.forEach(function(cv){ if(cv&&cv._sym) drawSym(cv,cv._sym); }); } }; sheetAr.src='assets/argentinasprites2026.png';
  ...
  function drawSym(cv, sym){ const x=cv.getContext('2d'); x.imageSmoothingEnabled=false; x.clearRect(0,0,cv.width,cv.height); cv._sym=sym; const idx=SPRITE_ORDER.indexOf(sym); var _ar=(state.palette==='argentina' && sheetArReady); var _sh=_ar?sheetAr:sheet; if(idx<0||(!_ar&&!sheetReady)) return; var _tw=(_sh.naturalWidth||(SPRITE_ORDER.length*32))/SPRITE_ORDER.length, _th=_sh.naturalHeight||32; x.drawImage(_sh, idx*_tw,0,_tw,_th, 0,0,cv.width,cv.height); }
  ...
  var PF_TEMANOM={synthwave:'MORADO', gameboy:'GAMEBOY', nes:'AZUL', vapor:'VAPOR', lava:'LAVA', mono:'MONOCROMO', galaxy:'GALAXY', casino:'CASINO CLASSIC', argentina:'ARGENTINA 🇦🇷'};
  ...
  function finish(t, isRespin, keepMask){
    ...
    if(bones>0){ if(!isRespin && respinProcs()){ respin(wasHeld); return; } combo=0; _tripleStreak=0; const _p=boneEscPct(); const pen=Math.max(1,Math.round(state.bet*bones*(1+_p)*petMod('boneDmg'))); var _bprev=state.credits; state.credits=Math.max(0,state.credits-pen); var _bloss=_bprev-state.credits; state.life.boneHits=(state.life.boneHits||0)+1; if(_bloss>(state.life.maxBoneLoss||0)) state.life.maxBoneLoss=_bloss; if(_bloss>=250000 && state.credits>=MIN_BET) state.life.survivedBigBone=true; updateUI(); setCombo(); setMsg('-'+fmt(pen)+'$','bad'); const cab=document.getElementById('cabinet'); cab.classList.add('bad'); setTimeout(function(){cab.classList.remove('bad');},600); sBad(); saveState(); checkBroke(); return; }
  ...
  function canLoan(){ return loanAvailable() && (state.credits+state.bank) < MIN_BET; }
  function takeLoan(){ if(spinning) return; if(!loanAvailable()){ setMsg('LÍMITE DE PRÉSTAMOS ($'+fmt(creditLimit())+')',false); return; } if((state.credits+state.bank)>=MIN_BET){ setMsg(state.bank>0?('Primero retirá tu plata del banco (tenés $'+fmt(state.bank)+' guardados).'):('Solo podés pedir préstamo si no te alcanza para la apuesta mínima ($'+MIN_BET+')'),false); return; } state.credits+=LOAN; state.debt+=Math.round(LOAN*(1+LOAN_FEE)); updateUI(); renderBank(); setMsg('PRÉSTAMO: +$'+fmt(LOAN)+' (debés $'+fmt(Math.round(LOAN*(1+LOAN_FEE)))+')',false); saveState(); }
  ...
  function renderBank(){
    ...
    if(!v){
      ...
      var _bnote=(state.bank>=bankCap())?'¡Caja fuerte llena! No genera interés. Subí el tope con 🔒 Tope del banco (en el Limbo).':(((state.credits+state.bank-state.debt)>=capValue())?'Llegaste al 🏔️ Techo de plata: el interés no crece más hasta que subas el Techo (en el Limbo).':'Subí el tope con 🔒 Tope del banco (en el Limbo).'); h+='<div class="bank-note">'+_bnote+'</div>';
    }
  ...
  function interestRate(){ return INT_RATES[Math.min(state.interestLevel||0, INT_RATES.length-1)] * (1 + 0.20*(state.meta.banker||0)) * petMod('interest'); }
  ...
  function accrueInterest(){ if((state.interestLevel||0)<=0 || state.bank<=0) return; var _cap=bankCap(); if(_cap>0 && state.bank>=_cap) return; state.intSpins=(state.intSpins||0)+1; if(state.intSpins>=interestInterval()){ state.intSpins=0; var g=Math.floor(state.bank*interestRate()); g=Math.min(g, state.bet*20); /* anti-farmeo: el interés no puede superar 20x tu apuesta, así no se farmea un banco enorme apostando el mínimo */ if(_cap>0) g=Math.min(g, _cap-state.bank); g=Math.min(g, capValue()-(state.credits+state.bank-state.debt)); if(g>0){ state.bank+=g; state.life.gotInterest=true; } } }
  ...
  function limboEarn(_basis){ var _b=(typeof _basis==='number')?_basis:(state.credits+state.bank-state.debt); var _div=3000*Math.sqrt(capValue()/1e6); var base=Math.max(0, Math.floor((_b-(state.runStartWorth||START))/_div))+(state.runJackpots||0)*5; return Math.round(base*(1+0.03*(state.meta.ectoboost||0))*petMod('ecto')); }
  ...
  var _afterCat=null; var _petTimer=null; function petLines(){ var ap=activePet(); return (ap&&ap.lines&&ap.lines.length)?ap.lines:['Zzz']; } function petTalk(){ var pet=document.getElementById('pet'), b=document.getElementById('petBubble'); if(!pet||!b) return; var arr=petLines(); var line=arr[Math.floor(Math.random()*arr.length)]; b.textContent=line; b.classList.add('show'); pokePet(); if(typeof sfx==='function') sfx(620+Math.random()*120, 0.03, 0.06, 'sine'); clearTimeout(_petTimer); _petTimer=setTimeout(function(){ b.classList.remove('show'); }, 2200); } var _pibbleSeq=[], _pibbleStep=0; function updatePet(){ var pet=document.getElementById('pet'); if(!pet) return; var img=document.getElementById('petImg'); var ap=activePet(); if(ap){ if(img){ var _b=petImgBase(ap); img.onerror=function(){ if(img.getAttribute('src')!==_b){ img.src=_b; } else { var p=document.getElementById('pet'); if(p) p.style.display='none'; } }; img.src=petImgSrc(ap); img.alt=ap.name; } pet.title=ap.name; pet.style.display='block'; } else { pet.style.display='none'; } } var _petLifeT=null, _pokeT=null; function pokePet(){ var pet=document.getElementById('pet'); if(pet){ clearTimeout(_pokeT); pet.classList.remove('poke'); void pet.offsetWidth; pet.classList.add('poke'); _pokeT=setTimeout(function(){ pet.classList.remove('poke'); }, 520); } } function petSay(txt,ms){ var b=document.getElementById('petBubble'); if(b){ b.textContent=txt; b.classList.add('show'); clearTimeout(_petTimer); _petTimer=setTimeout(function(){ b.classList.remove('show'); }, ms||1800); } } function petLifeTick(){ var pet=document.getElementById('pet'); var vis = activePet() && pet && pet.style.display!=='none' && !document.querySelector('.overlay.open'); if(vis){ if(Math.random()<0.5){ pokePet(); } else { var _arr=petLines(); petSay(_arr[Math.floor(Math.random()*_arr.length)], 1700); } } scheduleNextPetLife(); } function scheduleNextPetLife(){ clearTimeout(_petLifeT); _petLifeT=setTimeout(petLifeTick, 7000+Math.random()*9000); } function startPibble(){ _pibbleSeq=[ {msg:'...', btn:'▶ ¿...?'}, {msg:'WASH MY BELLY|WAAASHH MY BELLY', btn:'🫧 lavarle la pancita'}, {msg:'CLEAN MY BELLY', btn:'🧼 limpiarle la pancita'}, {msg:'Yaaay! :D|*Mr.PIBBLE quedó limpio y feliz*', btn:'▶ awww 🤍'} ]; _pibbleStep=0; _afterCat=function(){ unlockPibble(); }; showCat(_pibbleSeq[0].msg, 'pibble'); var cont=document.getElementById('catOk'); if(cont) cont.textContent=_pibbleSeq[0].btn; } function unlockPibble(){ var nuevo=!state.pibble; if(nuevo){ state.pibble=true; if(!state.pets) state.pets={owned:{},active:null}; if(!state.pets.owned) state.pets.owned={}; state.pets.owned.pibble=true; if(!state.pets.active) state.pets.active='pibble'; saveState(); updatePetsIcon(); } updatePet(); var pm=document.getElementById('pfCodeMsg'); if(pm){ pm.textContent= nuevo ? '🐽 ¡Mr.PIBBLE ahora vive en tu pantalla principal! Tocalo para acariciarlo 🤍' : 'Mr.PIBBLE ya está con vos 🐽'; pm.className='pf-code-msg ok'; } }
  ...
  function gambleResult(t){ spinning=false; setSpinUI(false); gambling=false; cur=t.slice(); const M=gambleMult(t); if(M>0){ var gain=gambleAmount*(M-1); if(state.debt>0){ var _dp=Math.min(gain,state.debt); state.debt-=_dp; gain-=_dp; } state.credits+=gain; gambleAmount*=M; if(gambleAmount>((state.life&&state.life.bestGamble)||0)) state.life.bestGamble=gambleAmount; updateUI(); setMsg('🎲 x'+M+'!  $'+fmt(gambleAmount),'big'); celebrate(M>=10); saveState(); showGamble(); } else { const lost=gambleAmount; state.credits-=gambleAmount; gambleAmount=0; updateUI(); setMsg('🎲 PERDISTE $'+fmt(lost),'bad'); sBad(); hideGamble(); saveState(); checkBroke(); } }
  ...
  var _ptc=document.getElementById('petsClose'); if(_ptc) _ptc.addEventListener('click', closePets);
  var _ptm=document.getElementById('petsModal'); if(_ptm) _ptm.addEventListener('click', function(e){ if(e.target===this) closePets(); });
  var _pti=document.getElementById('petsIcon'); if(_pti){ _pti.addEventListener('click', openPets); }
  setTimeout(maybeAutoDaily, 1500);
  updateDailyIcon();
