// Kitty's Slots Deluxe · DESARROLLO (v1.1-dev) · para TESTEAR, no subir
  ...
  const LOAN=1000, START=3000, JP_BASE=10000, JP_MULT=250, KEY='kitty-slots-deluxe', INTEREST=0.03, LOAN_FEE=0.1, SKILL_UNLOCK=5000, CAJA_COST=2000;
  ...
  function winMult(){ return 1 + 0.25*state.upg.win + 0.05*((state.meta&&state.meta.luck)||0); }
  function rebuildPool(){ pool.length=0; luckyPool.length=0; var slv=Math.max(1,state.superLevel||1); var k=1+(slv-1)*0.02; SYMS.forEach(function(s){ let w=WEIGHTS[s]; if(s==='🐱') w+=state.upg.cat*3; if(s==='🦴') w=Math.max(2,w-state.upg.bone*3); for(let i=0;i<w;i++) pool.push(s); let m=1; if(s==='🐱') m=10; else if(s==='🐟'||s==='💎') m=4; else if(s==='7️⃣') m=3; else if(s==='🍒'||s==='⭐') m=2.5; else if(s==='🔔') m=1.5; var em=1+(m-1)*k; var lw=Math.round(w*em); for(let i=0;i<lw;i++) luckyPool.push(s); }); }
  ...
  function applySave(d){
    ...
    if(typeof state.meta.spirits!=='number') state.meta.spirits=0; if(typeof state.meta.chest!=='number') state.meta.chest=0; if(typeof state.meta.credit!=='number') state.meta.credit=0; if(typeof state.meta.luck!=='number') state.meta.luck=0;
  ...
  async function initFirebase(){
    ...
    try{
      ...
      fbReady=true;
      _fbAuthMod.onAuthStateChanged(fbAuth, function(u){ fbUser=u||null; renderAccount(); if(fbUser){ cloudLoad(); advancePastLogin(); } });
    }catch(e){
      fbReady=false;
  ...
  const TXT_LOGIN="¡MIAU! 🐱 ANTES DE EMPEZAR, UN CONSEJO:||TOCÁ EL BOTÓN 👤 DE ARRIBA A LA DERECHA Y CREÁ TU CUENTA.||ASÍ GUARDÁS TU PROGRESO EN LA NUBE — SI NO, TU PARTIDA QUEDA SOLO EN ESTE APARATO.||¡Y PRONTO VA A HABER UN RANKING DE LOS QUE MÁS PLATA JUNTAN! 👑 ¿VAS A ESTAR EN EL TOP?";
  const HELP_TOPICS=[ {id:'intro',label:'🎮 Cómo jugar',text:TXT_INTRO}, {id:'cuenta',label:'👤 Cuenta y progreso',text:TXT_LOGIN}, {id:'bank',label:'🏦 Banco y préstamos',text:TXT_BANK} ];
  ...
  function closeCat(){ var m=document.getElementById('catModal'); if(m) m.classList.remove('open'); catMode=false; setCatLogin(false); }
  ...
  var META={ chest:{name:'🪙 Cofre inicial', desc:'Empezás con +$500 por nivel', max:10, cost:function(l){return 3+l*2;}}, credit:{name:'🏦 Crédito inicial', desc:'Arrancás con más préstamos disponibles', max:5, cost:function(l){return 5+l*4;}}, luck:{name:'✨ Patita de la suerte', desc:'+5% a TODO lo que ganás, para siempre', max:8, cost:function(l){return 4+l*3;}} };
  var _lastEarned=0;
  function gameOver(){ if(document.getElementById('limbo').classList.contains('open')) return; var earned=Math.max(1, Math.floor((state.peak||0)/1000))+(state.runJackpots||0)*5; state.meta.spirits+=earned; _lastEarned=earned; renderLimbo(); document.getElementById('limbo').classList.add('open'); saveState(); }
  function renderLimbo(){ var el=document.getElementById('limboBody'); if(!el) return; var h=''; h+='<div class="limbo-earn">Conseguiste <b>+'+_lastEarned+' 👻</b> en esta partida</div>'; h+='<div class="limbo-spirits">👻 Espíritus: <b>'+state.meta.spirits+'</b></div>'; h+='<div class="shop-section">MEJORAS PERMANENTES</div>'; Object.keys(META).forEach(function(k){ var u=META[k], lv=state.meta[k]||0, maxed=lv>=u.max, cost=u.cost(lv); var can=!maxed && state.meta.spirits>=cost; h+='<div class="bank-row"><div class="info">'+u.name+'<span class="ds-sm">'+u.desc+' · Nivel '+lv+'/'+u.max+'</span></div><button class="bank-btn lm-buy" data-k="'+k+'"'+((maxed||!can)?' disabled':'')+'>'+(maxed?'MÁX':(cost+' 👻'))+'</button></div>'; }); h+='<button class="limbo-go" id="limboGo">🔄 NUEVA PARTIDA</button>'; el.innerHTML=h; var bs=el.querySelectorAll('.lm-buy'); for(var i=0;i<bs.length;i++){ bs[i].addEventListener('click', function(){ buyMeta(this.getAttribute('data-k')); }); } document.getElementById('limboGo').addEventListener('click', startRun); }
  function buyMeta(k){ var u=META[k], lv=state.meta[k]||0; if(lv>=u.max) return; var cost=u.cost(lv); if(state.meta.spirits<cost) return; state.meta.spirits-=cost; state.meta[k]=lv+1; renderLimbo(); saveState(); }
  function startRun(){ state.credits=START+(state.meta.chest||0)*500; state.jackpot=JP_BASE; state.bet=25; state.debt=0; state.bank=0; state.vault=false; state.creditLevel=Math.min((state.meta.credit||0), CREDIT_LIMITS.length-1); state.interestLevel=0; state.intSpins=0; state.upg={win:0,combo:0,cat:0,bone:0}; state.abilities={hold:false,gamble:false}; state.holdLevel=0; state.superLevel=0; state.skillsUnlocked=false; state.bankUnlocked=false; state.peak=state.credits; state.runJackpots=0; combo=0; luckyUntil=0; superReadyAt=0; document.body.classList.remove('supersuerte'); rebuildPool(); renderBetPresets(); document.getElementById('betInput').value=25; updateUI(); setCombo(); renderPaytable(); renderShop(); renderHold(); updateSuperUI(); document.getElementById('limbo').classList.remove('open'); setMsg('NUEVA PARTIDA',false); saveState(); }
  ...
  const SUPER_CD=30;
  const SUPER_MAX=100;
  function superDur(lv){ return 6+Math.round(lv*0.2); }
  function superCost(lv){ return Math.round(3000*Math.pow(1.09,lv)/100)*100; }
